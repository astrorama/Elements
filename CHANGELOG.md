...
### Added
- Add the getPath getLocations and getVariableName python aliases
    - for the ElementsKernel.Configuration module
    - for the ElementsKernel.Auxliliary module

### Changed
- Update .jenkinsFile for EDEN-3.1 (Frédéric Leroux)
- Improve the build dependency of the sphinx-apidoc generation
    - The call is only done if there is a change in the files of the source
      directory
    - the output file used to check the timestamp is the modules.rst file
      generated by each call
- Modernization of the Log4CPP CMake find module

### Fixed
- Prevent the usage of the "-Wshadow=local" complation flag for clang
- Add the regexp to exclude /usr/sysroot/lib from the LD_LIBRARY_PATH
...
- Improve the cmake options passed to gcovr
    ...
    - the GCOVR_EXTRA_OPTIONS variable is set to "" by default and allows
...
-  Override the pybind11Tools.cmake file
    - remove the unconditional -flto for the optimized building. It can
     fail if the external linked library LTOs have been built with a
     slightly different compiler.
    - Use the ELEMENTS_LINKOPT cmake option to control this part.
...
- Add the TEST_XML_REPORT cmake option for the boost tests. It will
  generate XML files (instead of HRF) when set to ON. It is set to OFF
  by default.
...
- Add static functions to extract the current project information. They are located
  in the Elements::Project structure.
- Add static functions to extract the current module information. They are located
...
- Add the support for a custom Makefile. The file has to be pointed by the
  ...
  one.
- Add an utlity make library to be used in the CUSTOM_MAKEFILE.
- Add the CMake USER_SPHINX_NUMPYDOC option
    - it is ON by default
    - add the numpydoc_show_class_memebers=False in the conf.py to
...
- Add the generation of the git information in the manifest
...
- Fix the packaging of byte-compiled python files for FC30. The byte compilation
  is not done for files outside the /usr prefix
...
- Fix AddPythonProgram script which was creating an "scripts" subdirectory. The later was
