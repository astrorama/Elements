...
BOOST_AUTO_TEST_CASE(StorageConvertLower_test) {
  ...
  constexpr int64_t size_in_byte{1000};

  BOOST_CHECK(
      isEqual(storageConvert(static_cast<double>(size_in_byte), StorageType::Byte, StorageType::KiloByte), 0.977));
  BOOST_CHECK(isEqual(storageConvert<9>(static_cast<double>(size_in_byte), StorageType::Byte, StorageType::KiloByte),
                      0.9765625));

  BOOST_CHECK(
      isEqual(storageConvert(static_cast<double>(size_in_byte), StorageType::Byte, StorageType::MegaByte), 0.000954));
  BOOST_CHECK(isEqual(storageConvert<9>(static_cast<double>(size_in_byte), StorageType::Byte, StorageType::MegaByte),
                      0.000953674));
}
...
BOOST_AUTO_TEST_CASE(StorageConverterRounding_test) {
  ...
  BOOST_CHECK_EQUAL(storageConvert(size_in_byte, StorageType::Byte, StorageType::MegaByte), 0);
  BOOST_CHECK_EQUAL(storageConvert<9>(size_in_byte, StorageType::Byte, StorageType::MegaByte), 0);
}

BOOST_AUTO_TEST_CASE(StorageConvertUpper_test) {

  constexpr int64_t size{1};
  ...
  BOOST_CHECK(
      isEqual(storageConvert<9>(static_cast<double>(size), StorageType::MegaByte, StorageType::Byte), 1048576.0));
...
BOOST_AUTO_TEST_CASE(StorageConvertMetric_test) {
  ...
  BOOST_CHECK(
      isEqual(storageConvert<9>(static_cast<double>(size), StorageType::MetricKiloByte, StorageType::Byte), 1000.0));
  ...
  BOOST_CHECK(
      isEqual(storageConvert(static_cast<double>(size), StorageType::MetricMegaByte, StorageType::Byte), 1000000.0));
  BOOST_CHECK(
      isEqual(storageConvert<9>(static_cast<double>(size), StorageType::MetricMegaByte, StorageType::Byte), 1000000.0));
